...
proc sort data=target01 out=s_target01; by subjid tdtc visit method __vdate; run;

proc transpose data=s_target01 out=t_target01;
	by subjid tdtc visit method __vdate;
...
data target02;
	...
	tsite=ifc(scan(col1,2,"&dlm")^='.',strip(scan(col1,2,"&dlm")),'');
	tmeas=ifc(scan(col1,3,"&dlm")^='.',strip(scan(col1,3,"&dlm")),'');
	teval=ifc(scan(col1,4,"&dlm")^='.',strip(scan(col1,4,"&dlm")),'');
	tloc=ifc(scan(col1,5,"&dlm")^='.',strip(scan(col1,5,"&dlm")),'');
...
proc sql;
	...
	on a.subjid=b.subjid and a.__VDATE=b.__VDATE 
	;
quit;
...
data pdata.target(label='Target Tumor Lesions Assessments');
	retain SUBJID VISIT TDTC METHOD TNUM TEVAL TLOC TSITE TMEAS;
	keep SUBJID VISIT TDTC METHOD TNUM TEVAL TLOC TSITE TMEAS;
	set target04;
run;

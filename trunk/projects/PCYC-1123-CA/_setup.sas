...
%let    rawdatadir = %str(C:\Users\Yong\Documents\Nutstore\Janus\Projects\Patient Profile\pcyc-1123\Source Datasets\20150420);
%let newtransferID = %str(20Apr2015);
%let oldtransferID = %str(20Apr2015);
%let        genRTF = %str(N);
%let        genPDF = %str(Y);
%let        subset = %str(024-001);
%let      dlm4subj = %str( );
%let         rerun = %str(Y);
%let       compare = %str(N);
%let     debugMode = %str(Y);

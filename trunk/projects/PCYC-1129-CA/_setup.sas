...
%let    rawdatadir = %str(C:\Users\Yong\Documents\Nutstore\Janus\Projects\Patient Profile\pcyc-1129\Source Datasets\20150504);
%let newtransferID = %str(04May2015);
%let oldtransferID = %str(17Apr2015);
%let        genRTF = %str(N);
%let        genPDF = %str(Y);
%let        subset = %str(217-201 349-201 217-202);
%let      dlm4subj = %str( );
%let         rerun = %str(Y);
%let       compare = %str(Y);
%let     debugMode = %str(Y);

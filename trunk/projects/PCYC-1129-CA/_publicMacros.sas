...
%macro concatDate(year=, month=, day=, outdate=);
    ...
    call missing(__year, __month, __day);

    if &year > '' then __year  = strip(&year); else if &year="" then __year="UNK";
    if &month > '' then __month = put(strip(&month),$month.); else if &month="" then __month="UNK";
    if &day > '' then __day   = strip(&day); else if &day="" then __day="UNK";
	if __year="UNK" and __month="UNK" and __day="UNK" then &outdate="";
    else &outdate = strip(__year)||'-'||strip(__month)||'-'||strip(__day);
...
/*    call missing(__year, __month, __day);*/
/**/
/*    if &year > . then __year  = vvaluex("&year");*/
/*    if &month > . then __month = vvaluex("&month");*/
/*    __month = put(__month, $month.);*/
/*    if &day > . then __day   = vvaluex("&day");*/
/**/
/**/
/*    &outdate = ' ';*/
/**/
...
%macro dy(datevar, dyvar);
    ...
    if __rfstdtc = ' ' or __isdate = 0 then &dyvar = .;
    else do;
       __rfstdt__ = input(__rfstdtc, yymmdd10.);
         __date__ = input(&datevar, yymmdd10.);
         &dyvar = ifn(__date__>=__rfstdt__, __date__-__rfstdt__+1, __date__-__rfstdt__);
    end;
    drop __isdate __rfstdt__ __date__ __rfstdtc;
%mend dy;
...
/*    call missing(cycle, visit, unsseq);*/
    if VISDAY = ' ' then VISDAY = ' ';
    if visit = ' ' then visit = ' ';
    if unsseq = .  then unsseq = .;
end;
length visit2 $255;
visit2 = strip(strip(vvaluex('visit'))||' '||strip(vvaluex('VISDAY'))||' '||ifc(unsseq > ., strip(vvaluex('unsseq')), ' '));
label visit2="Visit";
drop visit visday unsseq;
%mend visit2;

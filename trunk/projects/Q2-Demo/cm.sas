...
/*    %concatDate(year=cmenyy, month=cmenmm, day=cmendd, outdate=cmendtc);*/
    ...
    length __dtitle1 __dtitle2 __dfootnote1 $255;
    __dtitle1 = strip(subject)||': dataset header 1';
    __dtitle2 = strip(subject)||': dataset header 2';
    __dfootnote1 = "&escapechar{style [foreground=green]"||strip(subject)||': dataset footer 1}';
    __dfootnote2 = "&escapechar{style [foreground=green]"||strip(subject)||': dataset footer 2}';
     
    if __EDC_TREENODEID = '4abda16a-c620-4f01-b0b6-31a582c852e1' then delete;

    keep __edc_treenodeid EDC_EntryDate subject cmcat cmtrt  cmindc dose unit freq route cmstdtc cmendtc cmongo
    __dtitle1 __dtitle2 __dfootnote1 __dfootnote2;
run;

proc sort data=cm;by subject cmstdtc cmendtc cmtrt;run;

data pdata.cm (label="Concomitant Medications");
    retain __edc_treenodeid __EDC_EntryDate subject cmtrt cmindc dose unit freq route cmstdtc cmendtc cmongo __dtitle1 __dtitle2 __dfootnote1 __dfootnote2;
    attrib
    dose    label="Dose"
    unit    label="Unit"
    ...
    ;
    set cm;
    keep __edc_treenodeid __EDC_EntryDate subject cmtrt  cmindc dose unit freq route cmstdtc cmendtc cmongo __dtitle1 __dtitle2 __dfootnote1 __dfootnote2;
run;

...
data lbchem_dm;
	...
	if index(labname1,'%')>0 and index(labname1,'BAZ MEGYEI KORHAZ ES EGYETEMI OKTATOKORHAZ, SZIKSZOI TELEPHELY, KOZPONTI LABORATORIUM')>0 
		then labname_='SQUALI CONT_JUDIT';
	else if index(labname1,'%')>0 and index(labname1,'BAZ MEGYEI ONKORMANYZAT KORHAZA ES EGYETEMI OKTATOKORHAZ, SZIKSZOI TELEPHELY, KOZPONTI LABORATORIUM')>0 
		then labname_='SQUALI CONT_JUDIT';
	else if index(labname1,'%')>0 and index(labname1,'CLINICAL BIOCHEMICAL LABORATORY OF SROC')>0 
		then labname_='SVERDLOVSK REGIONAL';
	else if index(labname1,'%')>0 and index(labname1,'JOSA ANDRAS OKTATOKORHAZ, KOZPONTI LABORATORIUM')>0 
		then labname_='JOSA ANDRAS';
	else if index(labname1,'%')>0 and index(labname1,'LABORATORY OF LENINGRAD REGIONAL CLINICAL')>0 
		then labname_='LENINGRAD';
	else if index(labname1,'%')>0 and index(labname1,'ST. PETERSBURG STATE MEDICAL UNIVERSITY')>0 
		then labname_='ST PETERSBURG_ORLOV';
	else if index(labname1,'%')>0 and index(labname1,'SYNLAB SZEKESFEHERVARI LABORATORIUMA 8001')>0 
		then labname_='SYNLAB';
...
/*	if low ^='' and high='' then rangelh=strip(low)||" -";*/
/*		else if low='' and high^='' then rangelh="  - "||strip(high);*/
/*		else if low^='' and high^='' then rangelh=strip(low)||" - "||strip(high);*/
/*		else if low='' and high='' then rangelh='  -';*/

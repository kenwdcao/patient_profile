...
data ae1;
    ...
    call missing(_dy);  %dy(aeendtc, yymmdd10.); aeendy = _dy; 
    if aestdy > . then aestdtc = strip(put(aestdy, best.)) || '/(' || strip(aestdtc) || ')';
...
data ae5;
    ...
    length aestdtc2 $10;
    *aestdtc2 = put(input(scan(aestdtc, 2, '()'), yymmdd10.), yymmdd10.);
    /* Ken Cao on 2014/10/28: foramt of AESTDTC was changed*/
    aestdtc2 = scan(aestdtc, 2, '()');
run;
...
%sort(indata = druglvl_2, sortkey = subjid aestdy aestdtc aeendy aeendtc aeterm);

...
merge dm_ex(in=in0) sites(in=in1) siteinv(in=in2);
    ...
    __TITLE2="&escapechar{style [fontweight=bold]Site Name: }"
             ...
             ;
    __TITLE3=  "&escapechar{style [fontweight=bold]Ibrutinib First Dose Date: }"||strip(exstdtc)
               ||"     &escapechar{style [fontweight=bold]Ibrutinib Last Dose Date: }"||strip(ldosedtc)
...
data pdata.DM(label='Demographics');
    ...
    label 
        BIRTHDTC = 'Birth Date'
        CBPN01 = 'Post menopausal'
        CBPN02 =    'Hysterectomy'
        CBPN04 = 'Tubal Ligation'
        CBPN03  = 'Bilateral oophorectomy'
        CBPN05  = 'Other'
        CBPNO   = 'Other Specify'
        RACE = 'Race'
    ;

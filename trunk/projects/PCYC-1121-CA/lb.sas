...
proc format;
    ...
    'RBC#K/MCL' = 'HEMATOLOGY# #Erythrocytes#RBC#10^12/L#BLOOD#0.001'
    'RBC#M/MM^3' = 'HEMATOLOGY# #Erythrocytes#RBC#10^12/L#BLOOD#1'
    'RBC#M/ MCL' = 'HEMATOLOGY# #Erythrocytes#RBC#10^12/L#BLOOD#1'
    'RBC#/PL' = 'HEMATOLOGY# #Erythrocytes#RBC#10^12/L#BLOOD#1'
...
data lb;
    length subject $13 rfstdtc $10 lborres $100;
    if _n_ = 1 then do;
    ...
    lbdtc timec lbcode rawcat rawunit rawtest result unit lbsymb _lbtest_ lborreso;
    ...
    if lborreso ^='' and strip(upcase(lborres)) ^='OTHER' and lborres ^='' then put "ERR" "OR:" lborres= lborreso = ;
    if lborreso ^='' and strip(upcase(lborres)) not in ('OTHER' '') then do; lborres = strip(lborres)||", other result: "||strip(lborreso);end;
    else do;
...
data lb_jn_master;
    ...
    by rawcat rawtest rawunit;
    if a;
    if a and not b then put "WARN" "ING: LB_MASETER: " subject= rawcat= rawunit= rawtest=;
...
** For 05/26/2015 transfer;
data lb_range(
        keep = tcd testcd test cat spec lbmethod sex__ symbol_age_low_ agelow agehigh age_units_ symbol_range_low low symbol_range_high high stresu low_other high_other other_units Comments);
    set source.lb_range;
    rename 
        age_low_ = agelow 
        age_high_=agehigh 
        pcyc_low_range_lbstnrlo = low 
        pcyc_high_range_lbstnrhi = high 
        lbcat = cat 
        lbtestcd = tcd 
        lbtest = test
        lbspec = spec 
        LOW_RANGE__Other_Units_ = low_other 
        high_range_other_units_ = high_other 
        from__other_units_ = other_units
        LBSTRESU_PCYC_Standard_Units = STRESU

;
    length testcd $8;
    testcd = ' ';
    *Modify, only keep Fasting PCYC-1121 uses ranges************;
    if lbtestcd = 'GLUC' and comments = 'Fasting PCYC-1122 uses ranges in NIH extract' then delete;
run;

**********Get Low, High from range dataset**************;
proc sort data = lb_range;
by tcd testcd test cat spec lbmethod sex__ symbol_age_low_ agelow agehigh stresu symbol_range_low low symbol_range_high high descending other_units;run;
 
proc sort data = lb_range nodupkey dupout =aa;
by tcd testcd test cat spec lbmethod sex__ symbol_age_low_ agelow agehigh stresu symbol_range_low low symbol_range_high high;
run;
...
data _null_;
    set lb_jn_range01;
    if tcd = '' and lblnd ='' and lbnr = '' and rawunit ^='' then put "WARN" "ING: LBRANGE: " subject= rawunit= RAWCAT= rawtest=;
run;
...
data pdata.chem1(label= 'Serum Chemistry Local');
    retain __edc_treenodeid __EDC_EntryDate  subject __ord visit lbdtc timec lbcode lblnd sodium potass chloride bicar bun creat glucose calcium ;
    keep __edc_treenodeid __EDC_EntryDate  subject __ord visit lbdtc timec lbcode lblnd sodium potass chloride bicar bun creat glucose calcium;
...
data pdata.chem2(label= 'Serum Chemistry Local (Continued)');
    retain __edc_treenodeid __EDC_EntryDate subject __ord visit lbdtc timec lblnd totprot albumin ast alt alk totbil ldh magnesium phosphate uricacid;
    keep __edc_treenodeid __EDC_EntryDate subject __ord visit lbdtc timec lblnd totprot albumin ast alt alk totbil ldh magnesium phosphate uricacid;
    set __stdchem(in=a) t_chem;
        by subject;
    if a then do;
        if _name_ = '__STDUNIT' then do;
...
data pdata.hem1(label= 'Hematology Local');
    retain __edc_treenodeid __EDC_EntryDate subject __ord visit lbdtc timec  lbcode lblnd wbc rbc HEMOG HEMAT PLATE NEUTRPH bands;
    keep __edc_treenodeid __EDC_EntryDate subject __ord visit lbdtc timec  lbcode lblnd  wbc rbc HEMOG HEMAT PLATE NEUTRPH bands;
...
data pdata.hem2(label= 'Hematology Local (Continued)'); ** Ken Cao on 2015/02/20: Fixed a typo(Contined --> Continued);
    retain __edc_treenodeid __EDC_EntryDate subject __ord visit lbdtc timec lblnd  lymph monocyte  eos basophils  lbacelyn lbacelsp;
    keep __edc_treenodeid __EDC_EntryDate subject __ord visit lbdtc timec lblnd lymph monocyte  eos basophils  lbacelyn lbacelsp;
    set __stdhem(in=a) t_hem;
        by subject;
    if a then do;
        if _name_ = '__STDUNIT' then do;
...
data pdata.coag(label= 'Coagulation Local');
    retain __edc_treenodeid __EDC_EntryDate subject __ord visit lbdtc timec 
    lbcode lblnd  pt aptt inr lbothyn lboth ;
    keep __edc_treenodeid __EDC_EntryDate subject __ord visit lbdtc timec 
    lbcode lblnd pt aptt inr lbothyn lboth ;

...
data pe_;
    ...
    orres=strip(peorres);
    if petest = 'Other' and orres ^= 'Not Done' and orres > ' ' then pecom = strip(peorreso)||': '||strip(pecom);
...
proc sort data=pe_; by subject pedtc; run;


data pdata.peabn(label='Physical Exam (Abnormal Finding)');
    retain __edc_treenodeid __EDC_EntryDate subject visit pedtc height weight test orres pecom  ;
    keep __edc_treenodeid __EDC_EntryDate subject visit pedtc height weight test orres pecom  ;
    set pe_;
    label
    weight='Weight'
    height='Height'

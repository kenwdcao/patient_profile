...
data cm2;
	length __label $300;
	set cm_stdt;
	if stdtc^='' then do; __label="Concomitant Medications !{style [foreground=&norangecolor](First Dose Date: "||strip(stdtc)||")}"
...
||" Date - First Dose Date if Date precedes First Dose Date}"; end;
	else do;__label="Concomitant Medications "||"!{style [foreground=&norangecolor](First Dose Date: NA)"||"}"; end;
run;

proc sort data=cm2; by SUBJECTNUMBERSTR ITMCMSTARTDT ITMCMENDDT; run;

data pdata.cm28(label='Concomitant Medications');

...
%if &nobs > 0 and
 (  %upcase(&dset) = COAGL or 
    %upcase(&dset) = CRCL or
    %upcase(&dset) = LBCOAG or
    %upcase(&dset) = HEMATL1 or
    %upcase(&dset) = HEMATL2 or
    %upcase(&dset) = SCHEML1 or
    %upcase(&dset) = SCHEML2 or
    %upcase(&dset) = TSHL or
    %upcase(&dset) = URINL or 
    %upcase(&dset) = TUMKRL1 or 
	%upcase(&dset) = TUMKRL2 or 
	%upcase(&dset) = TUMKRL3
)
%then %do;
    %labur;

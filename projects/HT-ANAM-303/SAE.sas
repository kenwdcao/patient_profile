...
data ae2;
	length __label $300;
	set ae1;
		if ITMAESER="No" then __label="Adverse Events";
    else if ITMAESER ^="" then __label="!{style [url='#dset39' linkcolor=white textdecoration=underline]Serious Adverse Events "||"!{style [foreground=&norangecolor] (See more details on Appendix 1)"||"}";
run;

data pdata.sae(label='Serious Adverse Events');
    retain  &globalvars3  __label ITMAESEQNUM DEATHCI ITMAEAUTPRF ITMAESERYES_CITMLIFETHR ITMAEADMDT_DTS ITMAEDISCHDT_DTS ITMAESERYES_CITMDISINC ITMAESERYES_CITMCONGANOM ITMAESERYES_CITMIMPMEDEVT
			ITMAEOTHPOSCAUS ITMAEIMPROVED ITMAEREAPPEAR ITMAELASTDOSEDT_DTS;
	keep    &globalvars3  __label ITMAESEQNUM DEATHCI ITMAEAUTPRF ITMAESERYES_CITMLIFETHR ITMAEADMDT_DTS ITMAEDISCHDT_DTS ITMAESERYES_CITMDISINC ITMAESERYES_CITMCONGANOM ITMAESERYES_CITMIMPMEDEVT
			ITMAEOTHPOSCAUS ITMAEIMPROVED ITMAEREAPPEAR ITMAELASTDOSEDT_DTS;
	set ae2;
run;

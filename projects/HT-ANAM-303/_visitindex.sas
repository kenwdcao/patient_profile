...
data _null_;
	...
	cards;
rd_frmdm rd_frmfollow rd_frmtarget rd_frmchem rd_frmchemuns rd_frmecg rd_frmecog rd_frmfaact 
rd_frmfacitf rd_frmhema rd_frmhemauns rd_frmhgs rd_frmhunger rd_frmkarnofsky rd_frmntarget_active 
rd_frmpe rd_frmtarget_scttar2tu_active rd_frmtmrass rd_frmvs rd_frmchemuns_sctchemo_active
;
...
%macro insertdov1(dset);
	proc sql;
		insert into _visit00
		select distinct subjectnumberstr, ifc(dov=.,'',scan(put(dov,is8601dt.),1,'T')), visitmnemonic, "%upcase(&dset)"
		from r301.&dset
		;
	quit;
...
data _null_;
	length dset $32;
	input dset :$ @@;
	if _n_=1 then call execute('%init1');
	call execute('%insertdov1('||dset||')');
	cards;
	...
	rd_frmhunger rd_frmkarnofsky rd_frmntarget1_active rd_frmntarget2_active rd_frmpe1 rd_frmpe2 rd_frmpe3 rd_frmpkecg
...
data _null_;
	length dset $32;
	input dset :$ @@;
	if _n_=1 then call execute('%init2');
	call execute('%insertdov2('||dset||')');
	cards;
rd_frmdm rd_frmbmiwthx rd_frmfollow rd_frmsurvival_active rd_frmtarget1 rd_frmtarget2 rd_frmchem rd_frmchemuns
rd_frmdxa rd_frmecg rd_frmecog rd_frmfaact rd_frmfacitf rd_frmhema rd_frmhemauns rd_frmhgs
...
%macro indexvst(indata=,out=);

/*proc sort data=_visit0 nodupkey; by subjectnumberstr visitmnemonic dov_c source; run;*/
proc sort data=&indata nodupkey; by subjectnumberstr visitmnemonic dov_c source; run;

data _visit1;
	set &indata(rename=source=in_source);
...
/*	if visitmnemonic='Day -14 to Day 0' then visitnum=0;*/
	if visitmnemonic in ('Day -14 to Day 0','Day -14 to 0') then visitnum=0;

	if visitmnemonic='Follow-up' then visitnum=14;
...
proc sort data=_visit2 out=pdata.&out; by subjectnumberstr visitnum; run;
%mend indexvst;

%indexvst(indata=_visit0,out=_visitindex);
%indexvst(indata=_visit00,out=_visitindex_r301);
%indexvst(indata=_visit000,out=_visitindex_r302);

...
%let sdsetn = pcyc1121ca_qlab1_20150519_1;
...
data lb;
    ...
    set source.&sdsetn(rename=(lbdtc = dtc)); 
...
data lb_jn_range;
    ...
    _lbtest2_ = _lbtest_;
    if index(_lbtest_, '/') > 0 then _lbtest_ = scan(_lbtest_,1,'/')||"/&splitchar"|| scan(_lbtest_,2,'/');
...
proc sort data = lb_jn_range(keep=subject _lbtest_ _lbtest2_ lborres unit low high) nodup out = __std0(drop=lborres);
    by subject _lbtest_ ;
    where lborres > ' ';
run;
...
proc sql;
    ...
    select distinct subject, _lbtest_, _lbtest2_, unit, low, high, count(*) as ntime
    from __std0
    group by subject, _lbtest_, unit;

    create table __std2 as
    select subject, _lbtest_, _lbtest2_, unit, low, high, ntime
    from __std1
...
data __std3;
    ...
    keep subject  _lbtest_ _lbtest2_ __stdunit __stdrange;
run;

*******************Modify __std3*****************;
proc sort data = lb_jn_range out = __std_mn(keep = subject _lbtest_) nodupkey; by subject _lbtest_ ;run;
...
data lb_nrind;
    ...
    keep rawcat rawtest lbdtc timec rawunit visit lbrefid lbgrpid  _lbtest_ _lbtest2_ unit lbstat lbreasnd lbmethod lbspec 
...
data pdata.imm1(label= 'Immunology');
    keep subject __ord visit lbdtc timec lbgrpid lbrefid cd3 cd3le  cd4 cd4le cd8  lbreasnd;
    retain subject __ord visit lbdtc timec lbgrpid lbrefid cd3 cd3le  cd4 cd4le cd8 lbreasnd;
...
data pdata.imm2(label= 'Immunology (Continued)');
    keep subject __ord visit lbdtc timec lbrefid  cd8le cd1656 cd1656le cd19 cd19le lbreasnd lbspec lbmethod lbnam ;
    retain subject __ord visit lbdtc timec lbrefid cd8le cd1656 cd1656le cd19 cd19le lbreasnd lbspec lbmethod lbnam ;
    set __stdimm(in=a) lb_nrind_nm;
        by subject;
    if a then do;
        if _name_ = '__STDUNIT' then do;

...
%macro formatDate2(date);
	length __Day $10 __Month $10 __Year $10 __Time __Time2 $10;
	* - > Get Year/Month/Day Component ;
	__Year=strip(scan(scan(&date,1,'T'),1,'-'));
	__Month=strip(scan(scan(&date,1,'T'),2,'-'));
	__Day=strip(scan(scan(&date,1,'T'),3,'-'));
	__Time2=scan(&date,2,'T');
	__Time=strip(substr(__Time2,1,5));
	* -> Numeric Month Value to 3-char value;
	__Result=0;
	%IsNumeric(InStr=__Month, Result=__Result);
/*	if upcase(__Month) not in ('UN','UNK','UK','NK','') then*/
	...
	* -> New Date Format;
	&date=strip(__Day)||ifc(__Month>'','-','')||strip(__Month)||ifc(__Year>'','-','')
			||strip(__Year)||ifc(__Time>'','T','')||strip(__Time);
%mend formatDate2;
...
%macro informatDate(date);
	...
	* -> New Date Format;
	A_&date=strip(__Day)||ifc(__Month>'','-','')||strip(__Month)||ifc(__Year>'','-','')
			||strip(__Year);end;
	else do;A_&date='';end;
%mend informatDate;

%macro TIME(date);
	if &date^='' then &date=strip(substr(&date,2,5));
%mend TIME;
...
%macro notInLowHigh(orres=,low=,high=,stresc=);
	...
	if __color>'' then &stresc='^{style [foreground='||strip(__color)||' fontweight=bold]'
								||strip(__orresc)||'}';

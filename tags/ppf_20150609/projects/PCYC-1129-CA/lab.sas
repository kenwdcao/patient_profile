...
proc sql;
    ...
    values('COAGULATION (LOCAL)', 'INR', 'INR', 'COAGULATION', 'Prothrombin Intl. Normalized Ratio', 'INR', 'RATIO', 1)
    ;
...
proc sort data=lb_master out=_hemalestd0 (keep=rawcat rawtest lbtest lbtestcd lbstresu) nodupkey;
    ...
    where lbtestcd in ('NEUT', 'NEUTB', 'LYM', 'MONO', 'EOS', 'BASO') ;
run;
...
data lab2;
    set lab1;
    if lbtestcd in ('NEUTLE', 'NEUTBLE', 'LYMLE', 'MONOLE', 'EOSLE', 'BASOLE') then 
        lbtestcd = substr(lbtestcd, 1, length(lbtestcd)-2);

run;

proc sort data=lab2; by __edc_treenodeid lbtestcd; run;
proc sort data=_hemale2; by __edc_treenodeid lbtestcd; run;

data lab3;
    merge lab2(in=a) _hemale2(in=b);
...
proc sql;
    ...
    from lab3 as a left join lbrange as b
    on a.lbcat = b.lbcat
...
proc sort data=lab4 out = _chkRange nodupkey;
...
proc sort data=lab5; by lbtest lbstresu; run;
proc sort data=source.lbgrade out=lbgrade; by lbtest lbstresu; run;


data lab6;
    merge lab5(in=a) lbgrade ;
        by lbtest lbstresu;
        if a;
    array gl(4) gl1-gl4;
    array gh(4) gh1-gh4;
...
data lab7;
    set lab6(rename=(lbstresu=__lbstresu));

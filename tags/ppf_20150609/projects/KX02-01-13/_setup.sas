...
%let rawdatadir               =       %str(Q:\Files\C113\ARG\KX-02\cdisc\dev\data\raw\Kamrab Data 16Jun2014\sas_rename);
%let studyid                  =       %str(KX02-01-13);
%let demodset                 =       %str(pdata.dm);
%let subjectvar               =       %str(subjid);
%let subset                   =       %str(01-503);

%let dlm4subj                 =       %str( );
%let newtransferID            =       %str(16JUN2014);
%let escapechar               =       %str(^);
%let SkipRawDataProcessing    =       %str(Y);
%let debugMode                =       %str(Y);

%let outputdir                =       %str(Q:\Files\CDM\Patient Profile\output\KX02-01-13);
...
%let sectionheadercolor       =       %str(#000000);

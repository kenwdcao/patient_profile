...
proc sql;
   ...
   /**modified by LDG 2015-04-23**/
    ...
    values('COAGULATION', 'INR', 'UNITLESS', 'COAGULATION', 'Prothrombin Intl. Normalized Ratio', 'INR', ' ', 1)
...
proc sql;
    ...
    from lab2 as a left join lbrange as b
    on a.lbcat = b.lbcat
...
proc sort data=lab3 out = _chkRange nodupkey;
...
proc sort data=lab4; by lbtest lbstresu; run;
proc sort data=source.lbgrade out=lbgrade; by lbtest lbstresu; run;


data lab5;
    merge lab4(in=a) lbgrade ;
        by lbtest lbstresu;
    array gl(4) gl1-gl4;
    array gh(4) gh1-gh4;
    array l(4) l1-l4;
...
**** For Hema ---LE tests;
proc sort data=lbrange out=_hemastresu(keep=lbtestcd lbstresu) nodupkey;
...
data _hemale _hemawbc(rename=(lbstresn=__wbc));
    ...
    keep __edc_treenodeid lbtestcd lbtest rawtest lbstresn lbstresc  lbstnr;
...
proc sort data=lab5; by __edc_treenodeid lbtestcd; run;
proc sort data=_hemale2; by __edc_treenodeid lbtestcd; run;


data lab6;
    ** Modified 2015-04-30;
    merge lab5(in=a where=(lbtestcd not in ('NEUTSGLE', 'NEUTBLE', 'LYMLE', 'MONOLE', 'EOSLE', 'BASOLE'))) _hemale2(in=b);
...
data lab7;
    length lbstresc $255;
    set lab6(rename=(lbstresu=__lbstresu lbstresc=__lbstresc));
...
proc transpose data=lab7 out=t_lab0;
...
proc sort data=lab7 nodupkey out=lab8_1(keep=subject rawcat lbtestcd rawtest2 lbstresu lbstnr ) ; 
    by subject rawcat lbtestcd rawtest2;
    where lbstresu ^= ' ' or lbstnr ^= ' ';
run;

proc sort data=lab7 nodupkey out=lab8_2(keep=subject rawcat lbtestcd rawtest2) ; 
    by subject rawcat lbtestcd rawtest2;
run;
...
proc transpose data=lab8 out=t_lab1;
    by subject rawcat;
    id lbtestcd;
    idlabel rawtest2;
    var lbstresu lbstnr;
...
data pdata.urinl(label='Urinalysis (Local)');
    retain __edc_treenodeid __edc_entrydate __edc_formlabel subject __ord visit2 lbdtc lbtmc lbtimunk spgrav ph gluc bili ketones blood prot;
    keep __edc_treenodeid __edc_entrydate __edc_formlabel subject __ord visit2 lbdtc lbtmc lbtimunk spgrav ph gluc bili ketones blood prot;
...
data pdata.tumkrl1(label='Tumor Markers (Local) - Breast Cancer');
    retain __edc_treenodeid __edc_entrydate __edc_formlabel subject __ord visit2 lbdtc lbtmc lbtimunk ca15_3ag ca2729ag;
    keep __edc_treenodeid __edc_entrydate __edc_formlabel subject __ord visit2 lbdtc lbtmc lbtimunk ca15_3ag ca2729ag;
...
data pdata.tumkrl2(label='Tumor Markers (Local) - NSCLC');
    retain __edc_treenodeid __edc_entrydate __edc_formlabel subject __ord visit2 lbdtc lbtmc lbtimunk ca125ag cea;
    keep __edc_treenodeid __edc_entrydate __edc_formlabel subject __ord visit2 lbdtc lbtmc lbtimunk ca125ag cea;
    ca125ag = ''; cea = '';
    set t_lab3;
    label ca125ag = 'Cancer Antigen 125'
          cea = 'Carcinoembryonic Antigen';
    where rawcat = 'TUMOR MARKERS - NSCLC';
run;

data pdata.tumkrl3(label='Tumor Markers (Local) - Pancreatic Cancer');
    retain __edc_treenodeid __edc_entrydate __edc_formlabel subject __ord visit2 lbdtc lbtmc lbtimunk ca19_9ag;
    keep __edc_treenodeid __edc_entrydate __edc_formlabel subject __ord visit2 lbdtc lbtmc lbtimunk ca19_9ag;

...
*<qs37--------------------------------------------------------------------------------------------------------;
/*%getVNUM(indata=source.RD_FRMFACITF, out=RD_FRMFACITF_3);*/

data fac301_01;
length assess $200;
	set r301.RD_FRMFACITF;
...
data fac302_01;
length assess $200;
	set r302.RD_FRMFACITF;
	%formatDate(ITMFACITFPERFDT_DTS); %informatDate(DOV);
	if ITMFACITFPERFDT_DTS ^='' then ASSESS =ITMFACITFPERFDT_DTS;
run;

data fac0102;
	set fac301_01 fac302_01;
	if strip(ITMFACITFASSESS_C) ='DONE';
run;

proc sort data=fac0102 out=s_fac0102; by SUBJECTNUMBERSTR DOV ASSESS; RUN;

data fac0102_01;
	length visitnum 8 VISITMNEMONIC $60;
	set s_fac0102(drop=VISITMNEMONIC);
...
data RD_FRMFACITF_3;
length VISITMNEMONIC $60;
	set RD_FRMFACITF_3(rename=VISITMNEMONIC=_VISITMNEMONIC);
		VISITMNEMONIC=_VISITMNEMONIC;
run;
...
data fac03_02;
	merge fac03_01(in=a)  fac0102_01(in=b);
	by SUBJECTNUMBERSTR;
	if a;
run;

proc sort data= fac03_02 out=fac03_03 nodupkey; by SUBJECTNUMBERSTR; run;

data fac;
	set RD_FRMFACITF_3 fac03_03;
run;

data fac_01;
	set fac (rename=(ITMFACITFNDAENUM=_ITMFACITFNDAENUM ITMFACITFASSESS=_ITMFACITFASSESS));
...
proc sort data=fac_01 out=s_qs01; by  SUBJECTNUMBERSTR  VISITMNEMONIC dov A_DOV ASSESS visitnum ;run;
...
data pdata.qs37(label='Quality of Life - FACIT-F');
	retain  __label SUBJECTNUMBERSTR qstest n5  _4 _8 _12  A QSTEST_ _n5_ _4_ _8_ _12_ ;
	keep  __label SUBJECTNUMBERSTR qstest  n5  _4 _8 _12  A QSTEST_ _n5_ _4_ _8_ _12_ ;
	merge qs05_ qs06_;

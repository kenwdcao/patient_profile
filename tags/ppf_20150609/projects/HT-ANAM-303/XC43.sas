...
proc sort data=xc02; by SUBJECTNUMBERSTR ITMCHEMOSTARTDT ITMCHEMOENDDT; run;


data pdata.xc43(label='Concomitant Chemotherapy/Immunotherapy');
	retain  &globalvars3 __label __ITEMSETIDX STATUS ITMCHEMOTHERAPY_ ITMCHEMOSTARTDT_DTS ITMCHEMOENDDT_DTS_ ITMCHEMOSTARTDOSE ITMCHEMOENDDOSE ITMCHEMOUNITS ITMCHEMOROUTE
			ITMCHEMOFREQ;
	keep    &globalvars3 __label __ITEMSETIDX STATUS ITMCHEMOTHERAPY_ ITMCHEMOSTARTDT_DTS ITMCHEMOENDDT_DTS_ ITMCHEMOSTARTDOSE ITMCHEMOENDDOSE ITMCHEMOUNITS ITMCHEMOROUTE
			ITMCHEMOFREQ;
	set xc02;
run;

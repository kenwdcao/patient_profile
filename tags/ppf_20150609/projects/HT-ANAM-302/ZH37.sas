...
data zhpe1;
	length __label $100;
	set zhpe;
	if ITMPE1NONDOMHAND ^="" then __label="Hand Grip Strength Test "||"^{style [foreground=&norangecolor]"||"(Non-dominant hand: "||strip(ITMPE1NONDOMHAND)||")"||"}";
		else __label="Hand Grip Strength Test "||"^{style [foreground=&norangecolor]"||"(Non-dominant hand: Unknown)"||"}";
RUN;
proc sort data=zhpe1; by SUBJECTNUMBERSTR dov; run;

data pdata.zh37(label='Hand Grip Strength Test');
	retain  &globalvars1  __label ITMHGSASSESS  ITMHGSRIGHTMEAS1_ ITMHGSRIGHTMEAS2_ ITMHGSRIGHTMEAS3_ ITMHGSRIGHTMAX_ ITMHGSLEFTMEAS1_ ITMHGSLEFTMEAS2_ ITMHGSLEFTMEAS3_ ITMHGSLEFTMAX_ __n1 __n2;
	keep    &globalvars1  __label ITMHGSASSESS  ITMHGSRIGHTMEAS1_ ITMHGSRIGHTMEAS2_ ITMHGSRIGHTMEAS3_ ITMHGSRIGHTMAX_ ITMHGSLEFTMEAS1_ ITMHGSLEFTMEAS2_ ITMHGSLEFTMEAS3_ ITMHGSLEFTMAX_ __n1 __n2;
	set zhpe1;
run;

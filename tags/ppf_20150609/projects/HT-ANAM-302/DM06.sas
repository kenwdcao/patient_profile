...
proc sql;
	...
	select a.*,b.Treatment_Group_Description
	from dm0 as a left join source.h302_treatment_group as b
...
run;
data dm060;
	length __STAT $100 __SEX $3 SUBJECTNUMBERSTR $20 lastvisit $100 __title2 $100 __treat $100;
	if _n_=1 then do;
	...
	**************add treatment information*****************;
	if Treatment_Group_Description='Placebo' then __treat='Placebo';
		else if Treatment_Group_Description='Anamorelin HCl 100 mg' then __treat='Anamorelin HCl';
		else if Treatment_Group_Description='' and ITMDMSTATUS='Subject in screening/screen failed with an SAE' then __treat='Screen Failure';
		else if Treatment_Group_Description='' and ITMDMSTATUS='' then __treat='NA';
	**********************************************************;
	...
	if ITMEOSCOMPLETE_C='COMPLETED' then __STAT='Completed: '||strip(ITMEOSETDT_DTS);
			...
			else if ITMEOSCOMPLETE_C='' and (ITMDMSTATUS='Randomized subject' OR strip(ITMDMSTATUS)='') then __STAT="^{style [foreground=&abovecolor] Screen Failure }";
			    else if ITMEOSCOMPLETE_C='' and ITMDMSTATUS='Subject in screening/screen failed with an SAE' then __STAT="^{style [foreground=&abovecolor] Screen Failure }";
	if AGE='' then AGE='NA';
...
data pdata.dm06(label='Demography');
	retain &globalvars1 ITMDMIFCDT_DTS ITMDMDOB_DTS ITMDMGENDER ITMDMETHNIC
		 RACE RACEOTH SITECOUNTRY ITMDMSTATUS ITMUPWRKFLW_CITMUPWRKFLW __title __title2 __SUBJECT __SEX __AGE __STAT; 
